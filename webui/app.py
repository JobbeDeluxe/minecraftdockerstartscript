...
def docker_published_ports():
    result = run_command(["docker", "ps", "--format", "{{json .Ports}}"], timeout=15)
    used = set()
    if not result["ok"]:
        return used
    for line in result["stdout"].splitlines():
        try:
            ports_text = json.loads(line)
        except json.JSONDecodeError:
            ports_text = line
        for match in re.finditer(r":(\d+)->\d+/(tcp|udp)", str(ports_text)):
            used.add((int(match.group(1)), match.group(2)))
    return used
...
def port_warnings(config):
    ...
    for label, port, proto in current:
        ...
        if port_is_open(port, proto):
            warnings.append(f"{label}: Port {port}/{proto} scheint auf dem Host bereits offen zu sein.")
    used_by_docker = docker_published_ports()
    for label, port, proto in current:
        if (port, proto) in used_by_docker:
            warnings.append(f"{label}: Port {port}/{proto} wird bereits von einem laufenden Docker-Container publiziert.")
...
def run_backend_action(config, action):
    if action == "apply":
        warnings = port_warnings(config)
        blockers = [w for w in warnings if w.startswith("eula:") or "kollidiert" in w or "doppelt" in w]
        if blockers:
