...
def backup_root(config): return Path(config.get("backup_root") or Path.home() / "minecraftdocker-backups")
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        try:
            ...
            elif len(parts) == 4 and parts[:2] == ["api", "servers"] and parts[3] == "backups": self.send_json({"backups": list_backups(read_server(parts[2]))})
    ...
    def do_POST(self):
        ...
        try:
            if parts == ["api", "servers"]: self.send_json(write_server(self.read_json()))
                ...
                body = self.read_json(); config = read_server(parts[2]); config["backup_file"] = body.get("file", ""); self.send_json(run_backend_action(config, "restore"))
            elif parts == ["api", "backups", "import"]:
                body = self.read_json(); source = Path(body.get("file", ""))
                if not source.exists(): self.send_json({"error": "backup not found"}, 404); return
                server_id = body.get("id", "").strip(); data = DEFAULT_SERVER.copy(); data.update({"id": server_id, "name": server_id, "container_name": f"mc-{server_id}", "data_dir": f"/opt/minecraft/{server_id}", "host_port": "25566", "rcon_host_port": "25576", "backup_file": str(source), "eula_accepted": True})
                saved = write_server(data); result = run_backend_action(saved | {"backup_file": str(source)}, "restore"); self.send_json({"message": "Backup als neues Serverprofil importiert.", "result": result, "server": saved})
            else: self.send_json({"error": "not found"}, 404)
        except Exception as exc: self.send_json({"error": str(exc)}, 500)
    def do_DELETE(self):
        parsed = urllib.parse.urlparse(self.path); parts = [p for p in parsed.path.split("/") if p]
